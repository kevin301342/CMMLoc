...
class CMMT_SC(nn.Module):
    def __init__(self, config):
        super(CMMT_SC, self).__init__()
        ...
        self.layer2 = nn.Linear(config.hidden_size, config.object_size)
...
class BertSelfAttention(nn.Module):
    ...
    def generate_cauchy_weight(self, props_len, width):
        ...
        w = 0.3183098861837907
        weight = w/(wid*(1+((weight-center)/wid)**2))            
    ...
    def forward(self, query_states, key_states, value_states, attention_mask=None):
        ...
        if self.wid is not None:
            cmm_mask = self.generate_cauchy_weight(attention_scores.shape[-1], self.wid)
            cmm_mask = cmm_mask.unsqueeze(0).unsqueeze(0)
            attention_scores = attention_scores_ori * cmm_mask
