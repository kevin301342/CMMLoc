...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory,relative_position,
                tgt_mask: Optional[Tensor] = None,
                memory_mask: Optional[Tensor] = None,
                tgt_key_padding_mask: Optional[Tensor] = None,
                memory_key_padding_mask: Optional[Tensor] = None,
        ...
        if not self.rm_self_attn_decoder:
            ...
            tgt2 = self.self_attn(q, k, value=v,relative_position=relative_position, attn_mask=tgt_mask,
        ...
        # q = q.view(num_queries, bs, n_model * 2)
        # k = k.view(hw, bs, self.nhead, n_model // self.nhead)
        # k_pos = k_pos.view(hw, bs, self.nhead, n_model // self.nhead)
        # k = torch.cat([k, k_pos], dim=3).view(hw, bs, n_model * 2)
        # k = k.view(hw, bs, n_model * 2)
        tgt2 = self.cross_attn(query=q,
                               key=k,
                               value=v,relative_position=None, attn_mask=memory_mask,
