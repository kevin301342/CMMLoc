...
class MultiheadAttention(Module):
    ...
    def forward(self, query, key, value,relative_position, key_padding_mask=None,
    ...
    Shape:
        ...
        if relative_position is None:
            ...
            if not self._qkv_same_embed_dim:
                return multi_head_attention_forward_with_position(
                    query, key, value,relative_position, self.embed_dim, self.num_heads,
                    self.in_proj_weight, self.in_proj_bias,
                    self.bias_k, self.bias_v, self.add_zero_attn,
                    self.dropout, self.out_proj.weight, self.out_proj.bias,
                    training=self.training,
...
def multi_head_attention_forward_with_position(query: Tensor,
                                 key: Tensor,
                                 value: Tensor,
    ...
    attn_output_weights = softmax(
        attn_output_weights, dim=-1)
    attn_output_weights = attn_output_weights + relative_position.to(torch.float32)
    attn_output_weights = softmax(
            attn_output_weights - attn_output_weights.max(dim=-1, keepdim=True)[0], dim=-1)
    attn_output_weights = dropout(attn_output_weights, p=dropout_p, training=training)
